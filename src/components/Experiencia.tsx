...
import { experiencia, formacao } from "@/data/perfil";
      ...
      <div className="grid gap-12 lg:grid-cols-[1.6fr_1fr]">
        <div>
          <ol className="relative space-y-8 border-l border-line pl-6">
            {experiencia.map((e) => (
              <li key={e.empresa} className="relative">
                <span
                ...
                <p className="mt-2.5 text-sm leading-relaxed text-muted text-pretty">{e.descricao}</p>
              </li>
            ))}
          </ol>
        </div>
        ...
        <div>
          ...
          <ul className="mt-5 space-y-5">
            {formacao.map((f) => (
              <li key={f.curso} className="border-t border-line pt-4">
