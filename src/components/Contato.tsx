...
/**
 * Sem formulário: um formulário exigiria mais um ponto de falha e uma caixa de
 * entrada a mais para vigiar. O mailto já chega classificado pelo assunto, que
 * é o que importa para separar recrutador de cliente.
 */
const assuntos = [
  {
...
function mailto(email: string, assunto: string, corpo: string) {
  return `mailto:${email}?subject=${encodeURIComponent(assunto)}&body=${encodeURIComponent(corpo)}`;
}

/**
 * Mostra `/in/martimtiburcio-dev` em vez da URL inteira. Derivado do link para
 * que trocar o perfil no admin não deixe um rótulo antigo apontando para lugar
 * nenhum.
 */
      ...
      <div className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
        <div className="rounded-2xl border border-line bg-elevated p-8 sm:p-12">
          ...
          <p className="mt-4 max-w-xl leading-relaxed text-muted text-pretty">
            Respondo em até um dia útil. Se for projeto, me conte o problema que precisa resolver —
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {assuntos.map((a, i) => (
              <a
                key={a.rotulo}
                href={mailto(perfil.email, a.assunto, a.corpo)}
